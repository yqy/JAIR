...
class NetWork():
    def __init__(self,n_hidden,embedding_dimention=50,feature_dimention=61):
        ...
        danwei = theano.shared(np.eye(extract_dimention, dtype=theano.config.floatX))
        ...
        Ws2_pre,heihei = init_weight(extract_dimention,n_hidden,pre="Ws2_pre_zp",ones=False)
        ...
        Ws2_post,heihei = init_weight(extract_dimention,n_hidden,pre="Ws2_post_zp",ones=False)
        ...
        cost = -(T.log((self.out*t).sum())) + + 0.5*f_norm
